...
import { selectCurrentJob } from '../../redux/slices/navigationSlice';
...
const EditJob = () => {
    ...
    const job_guid = useSelector(selectCurrentJob);
    const { user } = useSelector(getUserInfoState);
    const navigate = useNavigate();

    const { data, isLoading, isError } = useQuery({
        queryFn: async () => await baseGet(`/v1/job/by_guid/${job_guid}/`),
        queryKey: ["job"],
        enabled: !!job_guid, // Ensure the query only runs when job_guid is defined
    });
    ...
    useEffect(() => {
        if (data) {
            formik.setValues({
                ...
                agreed_budget: data.agreed_budget,
            });
        }
    }, [data]);
    ...
    const handleSubmit = async () => {
        setLoading(true);
        try {
            await basePatch(`/v1/job/update/${job_guid}/`, { ...formik.values, skills_required: skillsRequired, manager_guid: user.user.guid });
            navigate("/researcher");
        } catch (error: any) {
                    ...
                    <Grid container spacing={2}>
                        ...
                        <Grid item xs={12}>
                            <Autocomplete
                                ...
                                renderInput={(params) => (
                                    <TextField {...params} fullWidth label="Required skills" placeholder="Skills" />
                                )}
                            />
                        </Grid>
                        <Grid item xs={12} md={6}>
